...
public class PnDisplayAdultInfoGUI extends JPanel implements ActionListener{
	...
	JLabel lbColleaguelList = new JLabel("Colleaguel List");
	JComboBox<String> cbColleaguelList = new JComboBox<>();
	JLabel lbClassmateList = new JLabel("Classmate List");
	JComboBox<String> cbClassmateList = new JComboBox<>();
	JLabel lbChildList = new JLabel("Child List");
	JComboBox<String> cbChildList = new JComboBox<>();
	DefaultComboBoxModel modelFriend = new DefaultComboBoxModel<>();
	DefaultComboBoxModel modelColleaguel = new DefaultComboBoxModel<>();
	DefaultComboBoxModel modelClassmate = new DefaultComboBoxModel<>();
	DefaultComboBoxModel modelChild = new DefaultComboBoxModel<>();
	int _btWidth = 100;
	int _btHeight = 40;
	int _btDeleteX = 200;
	int _btDeleteY = 500;
	...
	public PnDisplayAdultInfoGUI(MainMenuGUI mainMenu, PnSelectPersonGUI pnSelect) {
		...
		this.add(lbColleaguelList);
		this.add(cbColleaguelList);
		this.add(lbClassmateList);
		this.add(cbClassmateList);
		this.add(lbChildList);
		this.add(cbChildList);
		this.add(btDelete);
		this.add(btCancel);
	...
	public void setPanel() {
		...
		lbColleaguelList.setBounds(_lblbX, lbFriendList.getY() + lbFriendList.getHeight() + _gapY, _lblbWidth, _lblbHeight);
		cbColleaguelList.setBounds(_lbvlX, lbColleaguelList.getY(), _cbWidth, _cbHeight);
		lbClassmateList.setBounds(_lblbX, lbColleaguelList.getY() + lbColleaguelList.getHeight() + _gapY, _lblbWidth, _lblbHeight);
		cbClassmateList.setBounds(_lbvlX, lbClassmateList.getY(), _cbWidth, _cbHeight);
		lbChildList.setBounds(_lblbX, lbClassmateList.getY() + lbClassmateList.getHeight() + _gapY, _lblbWidth, _lblbHeight);
		cbChildList.setBounds(_lbvlX, lbChildList.getY(), _cbWidth, _cbHeight);
